...
public class SwaggerMavenPluginTest extends AbstractMojoTestCase {
    ...
    public void testGeneratedSwaggerSpecYaml() throws Exception {
        executeAndAssertGeneratedSwaggerSpecYaml("This is a sample.", "/expectedOutput/swagger.yaml");
    }
    ...
    public void testSwaggerCustomReaderYaml() throws Exception {
        setCustomReader(mojo, "com.wordnik.jaxrs.CustomJaxrsReader");
        executeAndAssertGeneratedSwaggerSpecYaml("Processed with CustomJaxrsReader", "/expectedOutput/swagger.yaml");
    }
    ...
    private void assertGeneratedSwaggerSpecYaml(String description, String expectedOutput, String generatedFileName) throws IOException {
        String actualYaml = io.swagger.util.Yaml.pretty().writeValueAsString(
                new Yaml().load(FileUtils.readFileToString(new File(swaggerOutputDir, generatedFileName))));
    ...
    private void executeAndAssertGeneratedSwaggerSpecYaml(String description, String expectedOutput) throws MojoExecutionException, MojoFailureException, IOException {
        mojo.getApiSources().get(0).setOutputFormats("yaml");
        mojo.execute();

        assertGeneratedSwaggerSpecYaml(description, expectedOutput, "swagger.yaml");
    }
    ...
    public void testCustomModelConverterYaml() throws MojoFailureException, MojoExecutionException, IOException {
        ...
        executeAndAssertGeneratedSwaggerSpecYaml("This is a sample.", "/expectedOutput/swagger-with-converter.yaml");
    }
    ...
    public void testMultipleApiSourcesWithDifferentArtifactName() throws Exception {
        ...
        assertTrue(new File(swaggerOutputDir, "store.yaml").exists());
        assertGeneratedSwaggerSpecYaml("Only Store Resource", "/expectedOutput/only-store-resource.yaml", "store.yaml");
        assertTrue(new File(swaggerOutputDir, "user.json").exists());
        assertGeneratedSwaggerSpecJson("Only User Resource", "/expectedOutput/only-user-resource.json", "user.json");
